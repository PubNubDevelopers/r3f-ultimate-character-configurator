    ...
    <div className="md:rounded-t-lg bg-gradient-to-br from-black/30 to-indigo-900/20 backdrop-blur-sm drop-shadow-md flex flex-col py-6 gap-3 overflow-hidden">
      ...
      <div className="flex gap-2 overflow-x-auto noscrollbar px-6">
        ...
        {currentCategory?.assets.map((asset, index) => (
          <button
            key={`${currentCategory.id}-${asset.id}-${index}`} // ✅ Ensure each key is unique
...
export const UI = () => {
  ...
  const forceUpdate = useConfiguratorStore((state) => state.setMode);

  // Force update when category changes
  useEffect(() => {
    forceUpdate(UI_MODES.CUSTOMIZE);
  }, [currentCategory]);
  ...
  return (
    <main className="pointer-events-none fixed z-10 inset-0 select-none">
      ...
      >
        <img src="/images/wawasensei-white.png" className="w-40 animate-pulse" />
      </div>
      <div className="mx-auto h-full max-w-screen-xl w-full flex flex-col justify-between">
        <div className="flex justify-between items-center p-10">
          <a className="pointer-events-auto" href="https://lessons.wawasensei.dev/courses/react-three-fiber">
        ...
        <div className="md:px-10 flex flex-col">
          {mode === UI_MODES.CUSTOMIZE && (
            <>
              {currentCategory?.expand?.colorPalette && <ColorPicker />}
              <AssetsBox />
            </>
          )}
...
const ColorPicker = () => {
  ...
  // Ensure the category has a color palette and an asset is selected
  if (
    !currentCategory ||
    !currentCategory.expand?.colorPalette ||
    !customization[currentCategory.name] ||
    !customization[currentCategory.name].asset // 🚀 Hide if no asset is selected
  ) {
    return null;
  }
